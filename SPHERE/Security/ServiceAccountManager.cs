...
namespace SPHERE.Security
{
    ...
    /// 
    /// Traditional CNG containers are bound to the user of the computer. This secures them behind their log in, but the containers would be open to any other app on the users account. 
    /// (they couldn't be accessed for copy if export is turned off) however, a malicious app could gain access and sign an malicious block edit or add. By restricting the containers to 
    /// a service account and then restricting that access to the app alone, we reduce the attack surface drastically. 
    /// 
    /// The LSA is created using a dynamic user name and password.  Passwords are changed every time it is accessed. (which is at log on or creation most likely, or password resets)
    /// 
    /// Developers will only need to use ServiceAccountLogon()  it will check if an account exists and create one if it doesn't. 
    /// The service account name is created and stored in the registry while the account name and password are managed by the Credential Manager to rotate passwords and keep them in sync. 
    /// 
    ...
    /// 
    /// If access to this account is lost, the CNG Containers, and thus the keys are locked away inaccessible. Though you would still have the semi private key and BlockId for the contact so you could share and see it,  Editing it, or proving ownership would be impossible. 
    /// 
    /// </summary>
    public static class ServiceAccountManager
    {
        ...
        // Starts or creates the Service Account
        public static void ServiceAccountLogon()
        {
            try
            {
                ...
                using (var context = new PrincipalContext(ContextType.Machine))
                {
                    ...
                    if (user == null)
                    {
                        ...
                        SystemLogger.Log("Service account does not exist. Creating...");
                        ...
                        using (var newUser = new UserPrincipal(context))
                        {
                            ...
                            SystemLogger.Log($"Service account '{newUser.SamAccountName}' created successfully.");
                        }
                    }
                    else
                    {
            ...
            {
                SystemLogger.Log($"Error ensuring service account existence: {ex.Message}");
                throw;
            }
        }

        public static string ExportPrivateKey(KeyGenerator.KeyType keyType, string password = null)
        {
            string pass = ExportPrivateKeyFromContainer(keyType, password);
            return pass;
        }
            ...
            foreach (string password in passwordList)
            {
                try
                {
                    ...
                    if (LogonUser(username, ".", password, 5, 0, out userToken)) // LOGON32_LOGON_SERVICE
                    {
                        ...
                        {
                            SystemLogger.Log($"Successfully authenticated and impersonated service account '{username}' with the provided password.");
                    ...
                    {
                        SystemLogger.Log($"Authentication failed for service account '{username}' with the current password. Trying next...");
                    }
                }
                finally
                {
        ...
        internal static void ChangeServiceAccountPassword(string accountName, string newPassword, string currentPassword) //used to change the service account password.
        {
            try
            {
                ...
                using (var context = new PrincipalContext(ContextType.Machine))
                {
                    ...
                    SystemLogger.Log($"Password for service account '{accountName}' successfully updated.");
                }
            }
            catch (Exception ex)
            {
                SystemLogger.Log($"Error updating password for service account '{accountName}': {ex.Message}");
                throw;
            }
        }
        ...
        internal static void StoreKeyInContainerWithoutExport(byte[] key, KeyGenerator.KeyType keyType)
        {
            ...
            try
            {
                ...
                SystemLogger.Log("Private key stored securely with app-specific restrictions.");
            }
            catch (Exception ex)
            {
                SystemLogger.Log($"Error storing private key: {ex.Message}");
                throw;
            }
        }
        ...
        /// </summary>
        internal static void StorePrivateKeyInContainerWithExportPlainText(byte[] key, KeyGenerator.KeyType keyType, Password password)
        {
            ...
            try
            {
                ...
                SystemLogger.Log("Private key stored securely with password protection.");
            }
            catch (Exception ex)
            {
                SystemLogger.Log($"Error storing private key: {ex.Message}");
        ...
        internal static void StorePublicKeyInContainerWithExportPlainText(byte[] key, KeyGenerator.KeyType keyType)
        {
            string AppId = AppIdentifier.GetOrCreateAppId();
            string keyName = keyType.ToString();
            ...
            try
            {
                ...
                SystemLogger.Log("Public key stored securely and is exportable.");
            }
            catch (Exception ex)
            {
                SystemLogger.Log($"Error storing public key: {ex.Message}");
                throw;
            }
        }

        // The Encryption Keys are stored in Local CNG Containers.  Those containers are only accessible by the local Service account the App Creates and runs on. EXPORT AS CNGContainer.  MUCH SAFER OPTION BUT ONLY VIABLE TO OTHER WINDOWS DEVICES.
        internal static void StoreKeyInContainerWithExport(byte[] key, KeyGenerator.KeyType keyType, string password = null, bool isPublicKey = false)
        {
            string AppId = AppIdentifier.GetOrCreateAppId();
            string keyName = keyType.ToString();
            try
            ...
            {
                SystemLogger.Log($"Error storing key: {ex.Message}");
                throw;
            }
        }

        internal static byte[] UseKeyInStorageContainer(KeyGenerator.KeyType keyType)
        {
            try
            {
            ...
            try
            {
                ...
                // Ensure format matches creation
                using var cngKey = CngKey.Open(keyName, provider); 

                var format = keyType.ToString().Contains("Private")
                    ? CngKeyBlobFormat.Pkcs8PrivateBlob  
                    : CngKeyBlobFormat.EccPublicBlob;

                if (keyType.ToString().Contains("Private") && !keyType.ToString().Contains("Signature"))
                {
                    //  Only validate private keys to make sure they're ECDH
                    if (cngKey.AlgorithmGroup != CngAlgorithmGroup.ECDiffieHellman)
                    {
                        throw new InvalidOperationException($"Retrieved private key '{keyName}' is not an ECDH key! Instead, it is {cngKey.AlgorithmGroup}.");
                    }
                }
            ...
            {
                SystemLogger.Log($"❌ Error retrieving key '{keyName}': {ex.Message}");
                throw;
            }
        }

        internal static string ExportPrivateKeyFromContainer(KeyGenerator.KeyType keyType, string password = null)
        {
            ...
            {
                SystemLogger.Log($"Error exporting key: {ex.Message}");
                throw;
            }
        }
        ...
        internal static string ExportPublicKeyFromContainer(KeyGenerator.KeyType keyType)
        {
            string keyName = keyType.ToString();
            try
            {
        ...
        internal static bool KeyContainerExists(KeyGenerator.KeyType keyType)
        {
        ...
        internal static RSAParameters RetrievePrivateKeySecurely(KeyGenerator.KeyType keyType)
        {
            string keyName = keyType.ToString();
    ...
    internal static class AccountRestrictionManager
    {
        ...
        internal static void RestrictLogonRights(string accountName)
        {
            ...
            try
            {
                ...
                SystemLogger.Log($"Restricted logon rights for account: {accountName}");
            }
            catch (Exception ex)
            {
                SystemLogger.Log($"Error restricting logon rights: {ex.Message}");
            }
            finally
            {
