...
namespace SPHERE.Security
{
    ...
    /// find the last working password. Because we used it, it will be reset resolving the out of sync issue.
    /// 
    ...
    /// the service account password, it will be very difficult to gain access to the private keys stored under the account in CNG Containers.
    /// 
    /// </summary>
    internal static class CredentialManager
    {
        ...
        //Returns the user name and password [] containing the current and previous 5 passwords.
        internal static (string Username, string[] Password) GetOrCreateCredentials(string target)
        {
            var username = "";
        ...
        //Saves a credential.  (Primarily used for the first set up of the service account and initial save of the Credentials.  For existing Credentials CycleandUpdate is preferred. As the Service Account relies on cycling passwords to maintain a smaller attack window ) 
        internal static void SaveCredential(string target, string username, string[] password)
        {
            ...
            try
            {
                ...
                SystemLogger.Log($"Credential for '{target}' saved successfully.");
            }
            finally
            {
        ...
        //Used To change the password for the service account upon use and update the credential cycling the passwords down.(We keep the last 5 in case there is an error in updating it and new is not added correctly we ensure we have the previous ones it would still be set to.)
        internal static void CycleAndUpdateCredentialPassword(string target)
        {
            ...
            try
            {
                ...
                SystemLogger.Log($"Credential for '{target}' updated successfully with password history.");
            }
            finally
            {
        ...
        internal static (string Username, string[] Passwords) GetCredential(string target)
        {
            if (!CredRead(target, CRED_TYPE_GENERIC, 0, out IntPtr credentialPtr))
            {
        ...
        internal static bool CredentialExists(string target)
        {
        ...
        internal static void DeleteCredential(string target)
        {
            ...
            SystemLogger.Log($"Credential for '{target}' deleted successfully.");
        }

        //Used to verify the current service Account Password (Without revealing it)
        internal static bool VerifyCurrentPassword(string target, string currnetPassword)
        {
            if (CredentialManager.CredentialExists(target))
            {
                var (Username, Password) = CredentialManager.GetCredential(target);
